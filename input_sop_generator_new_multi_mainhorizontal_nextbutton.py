...
def display_sop_edit_form(sop_id):
    ...
    selected_facility_name = st_searchbox(options=facility_names, label="Facility Name", key=f"facility_name_{sop_id}")
...
def show_create_sop_section():
    ...
    st.write("作成する項目のメインとなる添付文書のPDFを使用してください。")
    main_title = st.text_input('Enter Main Title (SOP項目名):')
    subtitle = st.text_input("Enter Sub Title （SOP 使用試薬名）:")

    proceed = st.button("次へ")

    if proceed:
        if main_title and subtitle:
            # 重複チェック
            if check_sop_exists(main_title, subtitle):
                st.error("An SOP with the same Main Title and Sub Title already exists.")
            else:
                st.success("No existing SOP found with the same title. You can proceed.")

                uploaded_files = st.file_uploader("Upload SOP-related files", accept_multiple_files=True, type=['pdf', 'txt'], key=f"file_uploader_{main_title}_{subtitle}")
                ...
                if uploaded_files:
                    ...
                    with col2:
                        ...
                        if st.button("Generate SOP", key="generate_sop_button2"):
